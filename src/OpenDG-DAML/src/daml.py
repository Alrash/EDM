...
def train(train_source_iter_list, model, optimizer,
    ...
    # ##############
    mu_train = {domain: None for domain in range(len(train_source_iter_list))}
    std_train = {domain: None for domain in range(len(train_source_iter_list))}
    mu_val = {domain: None for domain in range(len(train_source_iter_list))}
    std_val = {domain: None for domain in range(len(train_source_iter_list))}
    # ##############
    ...
    for i in range(args.iters_per_epoch):
        ...
        for data_domain, train_source_iter in enumerate(train_source_iter_list):
            ...
            imgs.append(x_s)
            all_d.append(data_domain * torch.ones(args.batch_size).long().cuda())
        ...
        # ###################
        imgs, all_d = torch.cat(imgs, dim=0), torch.cat(all_d, dim=0)
        if args.level in [1]:
            aug, mu_train, std_train = mixup_distributions(imgs, all_d, mode='inter', momentum=args.mix_momentum,
                                                       mu_hist=mu_train, std_hist=std_train, alpha=args.mix_alpha)
        if args.level in [2]:
            aug, mu_train, std_train = mixup_distributions(imgs, all_d, mode='extra', alpha=args.mix_alpha,
                                                       momentum=args.mix_momentum if args.level == 2 or args.mix_momentum == 0 else 1,
                                                       mu_hist=mu_train, std_hist=std_train, ex_alpha=args.mix_ex_alpha)

        index = torch.randperm(aug.size(0)).long()
        aug, aug_coding = aug[index], all_one_hot_labels[index]
        f_aug, coding_aug = [], []
        for model_domain in range(3):
            _, fea = model(aug[model_domain*args.batch_size:(model_domain+1)*args.batch_size], domain=model_domain)
            f_aug.append(fea)
            coding_aug.append(aug_coding[model_domain*args.batch_size:(model_domain+1)*args.batch_size])
        # ###################
        ...
        for model_domain in range(3):
            ...
            mixup_dir_list = [args.mixup_dir2, args.mixup_dir2, args.mixup_dir2, args.mixup_dir2]
            ...
            mixup_features = torch.stack(
                [all_f_s_1[mix_indeces_1], all_f_s_2[mix_indeces_2], all_f_s_3[mix_indeces_3], f_aug[model_domain]])  # 4 * N * D
        ...
        imgs, all_d = torch.cat(imgs, dim=0), torch.cat(all_d, dim=0)
        if args.level in [1]:
            aug, mu_val, std_val = mixup_distributions(imgs, all_d, mode='inter', momentum=args.mix_momentum,
                                                           mu_hist=mu_val, std_hist=std_val, alpha=args.mix_alpha)
        if args.level in [2]:
            aug, mu_val, std_val = mixup_distributions(imgs, all_d, mode='extra', alpha=args.mix_alpha,
                                                           momentum=args.mix_momentum if args.level == 2 or args.mix_momentum == 0 else 1,
                                                           mu_hist=mu_val, std_hist=std_val, ex_alpha=args.mix_ex_alpha)
        ...
        for model_domain in range(3):
            ...
            mixup_dir_list = [args.mixup_dir, args.mixup_dir, args.mixup_dir, args.mixup_dir]
            ...
            mixup_features = torch.stack([all_f_s_1[mix_indeces_1], all_f_s_2[mix_indeces_2], all_f_s_3[mix_indeces_3], f_aug[model_domain]]) # 4 * N * D
            mixup_labels = torch.stack([all_one_hot_labels_1[mix_indeces_1],
                                        all_one_hot_labels_2[mix_indeces_2],
                                        all_one_hot_labels_3[mix_indeces_3],
                                        coding_aug[model_domain]])    # 4 * N * C
...
if __name__ == '__main__':
    ...
    parser.add_argument("--mix_momentum", type=float, default=0)
    parser.add_argument("--mix_alpha", type=float, default=0.5)
    parser.add_argument("--mix_ex_alpha", type=float, default=0.5)
    parser.add_argument("--level", "-l", type=int, default=0, help="0: basic, 1: +Inter, 2: +Extra")
