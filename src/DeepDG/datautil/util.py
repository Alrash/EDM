...
def random_pairs_of_minibatches(args, minibatches, aug=None):
    ld = len(minibatches)
    pairs = []
    if aug is not None:
        minibatches.append([])
    tdlist = np.arange(ld + (0 if aug is None else 1))
    txlist = np.arange(args.batch_size)
    for i in range(ld):
        if aug is not None:
            minibatches[-1] = [aug[0][i*args.batch_size:(i+1)*args.batch_size],
                               aug[1][i*args.batch_size:(i+1)*args.batch_size],
                               aug[2][i*args.batch_size:(i+1)*args.batch_size]]
